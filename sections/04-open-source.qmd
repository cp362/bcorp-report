...
#| fig-height: 2.75
python |> 
  filter(package == "shiny", year(date) > 2022) |> 
...
```{r}
#| label: plotnine
python |> filter(package == "plotnine", year(date) > 2022) |> growth_plot() + ggtitle("Plotnine")
```
...
```{r}
#| label: siuba
python |> filter(package == "siuba", year(date) > 2022) |> growth_plot() + ggtitle("Siuba")
```
